...
export class Task {
  ...
  setEventListeners(taskList) {
    console.log('tyt');
    this.buttons.forEach((button) => {
      const blockTitle = button.parentElement.querySelector('.tasks-list__title');
      console.log(blockTitle.textContent);
      if (blockTitle.textContent === taskList.title) {
        button.addEventListener('click', () => {
          this.writeNewTask(button.parentElement);
        });
      } else {
        button.addEventListener('click', () => {
          this.getAnotherTasks(button, blockTitle);
        });
      }
    });
  }
